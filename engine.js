...
// Authored, nonperiodic road elevations; shared by physics and every world-space prop.
const TERRAIN_POINTS=[[-800,720],[-100,710],[1350,330],[1710,330],[2810,690],[3550,835],[3910,820],[5100,390],[5570,375],[6900,860],[7430,785],[8670,350],[9110,350],[10200,730],[11020,504],[11800,504]];
function terrain(x){const a=TERRAIN_POINTS;if(x<=a[0][0])return a[0][1];for(let i=1;i<a.length;i++)if(x<=a[i][0]){const p=a[i-1],q=a[i],t=(x-p[0])/(q[0]-p[0]);return p[1]+(q[1]-p[1])*t*t*(3-2*t)}return a[a.length-1][1]}
function jumpY(from,to,p,lift){return terrain(from)+(terrain(to)-terrain(from))*p-Math.sin(Math.PI*p)*lift}
class Journey{
 ...
 boyY(){return this.action==='jump'?jumpY(this.jumpFrom,this.jumpFrom+78*2.3,Math.min(1,this.actionTime/2.3),92):terrain(this.x)}
 height(){return terrain(this.x)-this.boyY()}
 ...
 command(name){
  ...
  this.action=name;this.actionTime=0;this.lastSound=name;if(name==='jump')this.jumpFrom=this.x;
 ...
 dragonPose(){
  ...
  const p=Math.min(1,(m.t-.2)/m.duration),y=jumpY(m.from,m.to,p,m.height),lift=terrain(this.dragonX)-y;
  return {x:this.dragonX,y,height:lift,tuck:Math.sin(Math.PI*p)*.24,crouch:0,state:'air',radius:r};
 }
 updateDragon(dt){
...
root.Journey=Journey;root.ROUTE=ROUTE;root.terrain=terrain;if(typeof module!=='undefined')module.exports={Journey,ROUTE,terrain,TERRAIN_POINTS,jumpY};
